...
- A PostgreSQL `BEFORE UPDATE OR DELETE` row-level trigger rejects those two mutations with SQL state `55000`. It does not cover `TRUNCATE`, because PostgreSQL never fires row-level triggers for it, and that gap is left open deliberately: `TRUNCATE` is the only way a disposable database can be reset between integration tests once `DELETE` is blocked. A `TRUNCATE` is still not silent -- the count and head anchor on `digital_evidence` is untouched, so verification reports a chain-length mismatch. The guarantee is therefore tamper-evidence, not tamper-proofing: no schema-level guard survives a superuser or an owner who disables triggers first. Application-level restrictions reinforce it: an `@Immutable` entity with non-updatable columns, a minimal repository interface that publishes only saves, an existence check and deterministic reads, and no public endpoint that writes events.
